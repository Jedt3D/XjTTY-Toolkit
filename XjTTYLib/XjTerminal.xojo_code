...
Protected Module XjTerminal
	...
	#tag Method, Flags = &h0
		Sub GetSize(ByRef width As Integer, ByRef height As Integer)
		  ...
		  #If TargetMacOS Then
		    ...
		    Var ws As MemoryBlock
		    
		    Soft Declare Function ioctl Lib "libSystem.B.dylib" (fd As Int32, request As UInt64, ws As Ptr) As Int32
		    
		    // Try stderr (fd=2) and stdin (fd=0) before stdout (fd=1).
		    // Xojo's runtime wraps stdout through its own PTY sized 80x24, so
		    // ioctl(1) returns the wrong dimensions. fd=2 and fd=0 are typically
		    // not redirected and reflect the real controlling terminal size.
		    Var gotSize As Boolean = False
		    
		    // fd=2 (stderr) — best bet, rarely redirected
		    ws = New MemoryBlock(8)
		    If ioctl(2, &h40087468, ws) = 0 Then
		      Var tw As Integer = ws.UInt16Value(2)
		      Var th As Integer = ws.UInt16Value(0)
		    ...
		    If Not gotSize Then
		      ws = New MemoryBlock(8)
		      If ioctl(0, &h40087468, ws) = 0 Then
		        Var tw As Integer = ws.UInt16Value(2)
		        Var th As Integer = ws.UInt16Value(0)
		        If tw > 0 And th > 0 Then
		          width = tw
		    ...
		    // fd=1 (stdout) — only if it looks real (not the 80x24 Xojo PTY default)
		    If Not gotSize Then
		      ws = New MemoryBlock(8)
		      If ioctl(1, &h40087468, ws) = 0 Then
		        Var tw As Integer = ws.UInt16Value(2)
		        Var th As Integer = ws.UInt16Value(0)
		        If tw > 0 And th > 0 And Not (tw = 80 And th = 24) Then
		          width = tw
		          height = th
		          gotSize = True
		        End If
		    ...
		    // Last resort: stty via Shell. Cached for 250ms so the 30fps event loop
		    // doesn't spawn a process every tick.
		    If Not gotSize Then
		      Var now As Double = System.Microseconds
		      If mSizeCacheW > 0 And (now - mSizeCacheTime) < 250000 Then
		        width = mSizeCacheW
		        height = mSizeCacheH
		        gotSize = True
		      Else
		        Var sh As New Shell
		        sh.Execute("stty size < /dev/tty 2>/dev/null")
		        Var deadline As Double = System.Microseconds + 2000000
		        While sh.IsRunning And System.Microseconds < deadline
		          App.DoEvents(10)
		        Wend
		        Var result As String = sh.Result.Trim
		        If result.Length > 0 Then
		          Var parts() As String = result.Split(" ")
	...
	#tag Property, Flags = &h21
		Private mSizeCacheH As Integer
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mSizeCacheTime As Double
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mSizeCacheW As Integer
	#tag EndProperty
